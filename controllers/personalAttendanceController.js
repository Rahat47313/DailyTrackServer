...
const getAttendanceByMonth = async (req, res) => {
  ...
  try {
    const attendanceRecord = await PersonalAttendance.findOne({ "attendance.year": year });
...
const getAttendanceByDate = async (req, res) => {
  ...
  try {
    const attendanceRecord = await PersonalAttendance.findOne({ "attendance.year": year });
    if (!attendanceRecord || !attendanceRecord.attendance[year].months[month] || !attendanceRecord.attendance[year].months[month].days[day]) {
      return res.status(404).json({ error: "Attendance record not found" });
    }
    res.status(200).json(attendanceRecord.attendance[year].months[month].days[day]);
  } catch (error) {
    res.status(400).json({ error: error.message });
  }
};
